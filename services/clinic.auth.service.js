...
const clinicDao = require("../daos/clinic.dao");


class ClinicService {
  async registerService(req, res) {
    try {
      const { firstName, lastName, mobileNumber, email, clinicName } = req.body;

      if (!firstName && !lastName && !mobileNumber && !email && !clinicName) {
        log.error("Error from [Clinic SERVICE]: Empty request body");
      ...
      const clinicData = removeNullUndefined({ firstName, lastName, mobileNumber, email, clinicName });
      const clinicInfo = await clinicDao.createClinic(clinicData);
      if (!clinicInfo.data) {
      ...
      return res.status(200).json({
        ...
        // data: {
        //   clinic: {
        //     firstName: clinicInfo.data.firstName,
        //     lastName: clinicInfo.data.lastName,
        //     email: clinicInfo.data.email,
        //     mobileNumber: clinicInfo.data.mobileNumber,
        //     clinicName: clinicInfo.data.clinicName,
        //   },
        // },
      });
    } catch (error) {
      log.error("Error from [Clinic SERVICE]:", error);
      return res.status(500).json({
