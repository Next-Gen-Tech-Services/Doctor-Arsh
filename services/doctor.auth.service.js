...
const doctorDao = require("../daos/doctor.dao");
  ...
  async registerService(req, res) {
    try {
      ...
      if (!firstName && !lastName && !mobileNumber && !email) {
        log.error("Error from [Doctor SERVICE]: Empty request body");
      ...
      const doctorData = removeNullUndefined({ firstName, lastName, mobileNumber, email });
      const doctorInfo = await doctorDao.createDoctor(doctorData);
      ...
      return res.status(200).json({
        ...
        //     email: doctorInfo.data.email,
        //     mobileNumber: doctorInfo.data.mobileNumber,
        //   },
        // },
      });
    } catch (error) {
      log.error("Error from [Doctor SERVICE]:", error);
      return res.status(500).json({
